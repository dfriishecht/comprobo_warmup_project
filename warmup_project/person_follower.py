...
class PersonFollow(Node):
    def __init__(self):
        ...
        self.laser_sub = self.create_subscription(
            LaserScan, "scan", callback=self.follow_person, qos_profile=10
        )
    ...
    def get_angle(self, msg):
        ...
        self.current_angle = rad2deg(acos(w) * 2)
    ...
    def group_clusters(self, dists, max_gap):
        ...
        for dist in dists[1::]:
            if (
                abs(dist[1] - prev_dist) <= max_gap
            ):  # If crnt point is within tolerable gap distance, add it to crnt cluster
    ...
    def closest_cluster(self, clusters, window):  # change to max_dist
        ...
        for cluster in clusters:
            if cluster[0][1] <= window:  # Is the cluster in scan window?
                ...
                ):  # Is it larger than specified minimum size?
                    # If it is:
                    if len(cluster) > len(
                        crnt_largest
                    ):  # Is it larger than the currently largest cluster?
                        # If it is:
    ...
    def follow_person(self, msg):
        ...
        dists = (
            list(msg.ranges[45:0:-1]) + [0] + list(msg.ranges[359:314:-1])
        )  # Extract angle-appropriate lidar values
